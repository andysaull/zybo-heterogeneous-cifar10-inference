...
de la Zybo utilizada en este trabajo, el dispositivo es un Zynq-7010. La parte PS incluye un ARM
...
dispositivo. Según la hoja de datos de la familia Zynq-7000, el Zynq-7010 dispone de 17.600 LUT,
...
el diseñador describe el algoritmo en C o C++, y la herramienta genera una implementación RTL que
después puede integrarse como IP en Vivado.

En el caso de AMD/Xilinx, Vitis HLS sintetiza funciones C/C++ a RTL para implementarlas en la
...
CNN en una Zynq-7010, sigue siendo imprescindible leer los informes de síntesis, entender los
...
neuronal en C++ para HLS, hls4ml parte de modelos entrenados en herramientas habituales de machine
...
del grafo y genera código C++ sintetizable. La documentación de hls4ml describe esta separación
...
memoria en algunas capas, pero introduce FIFOs y lógica adicional que también cuentan para Vivado.
...
\section{IA en FPGAs con recursos limitados}
...
convolucional con muchas capas y canales, pero en una Zynq-7010 cada capa adicional se nota en el
...
\begin{itemize}
  ...
  \item \textbf{hls4ml:} conversión del modelo entrenado a código C++ sintetizable y generación del
    proyecto HLS.
  \item \textbf{Vitis HLS:} síntesis del código C++ a RTL y empaquetado del acelerador como IP.
...
del rendimiento de las CPUs. También se ha convertido en una estrategia industrial para reducir
latencia y consumo en cargas muy concretas.

\subsection{Microsoft Project Catapult}
Uno de los referentes más conocidos en el uso de FPGAs a gran escala es Project Catapult de
Microsoft. Este proyecto surgió para acelerar tareas de búsqueda y
clasificación en centros de datos \citep{putnam2014a}.
La idea principal era añadir FPGAs a los servidores para obtener aceleradores reconfigurables
...
\subsection{NVDLA}
NVDLA, anunciado por NVIDIA, representa otro enfoque: una arquitectura abierta y configurable para
...
A una escala más cercana a este trabajo, el repositorio \textit{BNN-PYNQ-ZYBO} adapta el proyecto
BNN-PYNQ \citep{finn} para la Zybo-Z7-10. No es exactamente la Zybo Legacy utilizada aquí, pero
sí trabaja con
una placa basada en el mismo tamaño de dispositivo, el Zynq-7010. El propio repositorio indica que
...
MNIST \citep{bnnPynqZybo}.

Este tipo de proyecto es útil como referencia porque confirma una idea que también aparece durante
este TFG: en un Zynq-7010, la clave no es intentar llevar una red grande sin cambios, sino diseñar
