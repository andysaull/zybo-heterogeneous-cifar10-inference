...
Digilent Zybo board, using the \ac{ARM} processor to control the application and the \ac{FPGA} to
...
an image from the \ac{ARM}, transfer it to the programmable logic through \ac{AXI}
\ac{DMA}, execute a
...
only training a \ac{CIFAR}-10 model, but making that model actually fit within a Zynq Z-7010.
Many configurations that seemed reasonable in terms of parameter count became unfeasible
after implementation in Vivado, especially due to \ac{LUT} usage and slice packing issues.
This part of the work made it clear that, on a small \ac{FPGA}, discussing only the model size
...
the complete Vivado/Vitis workflow. Bare-metal execution on the \ac{ARM} was validated, the
communication between the Processing System and the Programmable Logic was configured,
and \ac{AXI}-Lite and AXI \ac{DMA} were used. These steps were necessary to ensure that the
artificial intelligence component was not left as an isolated \ac{IP} block without real
system-level integration.

The matrix multiplication developed in Phase 3 served as an intermediate milestone. It
was a more controlled test case than the \ac{CNN}, but still sufficient to verify that Vitis
\ac{HLS} could generate competitive hardware and that the \ac{ARM}--\ac{DMA}--\ac{FPGA}
data path worked
correctly. In addition, the comparison with a \ac{VHDL} implementation helped assess the use
of \ac{HLS} from a more informed perspective. The tool does not remove the need to understand
the underlying hardware, but it does significantly reduce the effort required to reach a
functional parallel architecture.

In Phase 4, a complete \ac{CIFAR}-10 model was successfully generated and integrated. The
...
inference running on dedicated hardware inside a resource-constrained \ac{FPGA}.

Finally, the evaluation phase made it possible to measure the benefit of the
acceleration. \ac{FPGA} inference achieved an average execution time close to 2 ms, compared
with approximately 171 ms for the hls4ml-generated \texttt{C++} core running on the \ac{ARM}. This
...
communication cost with the \ac{IP} core.

\section*{Lessons learned}
\addcontentsline{toc}{section}{Lessons learned}
One of the clearest conclusions of the project is that \ac{HLS} simplifies development, but
...
fit on the \ac{FPGA}. In this regard, working with hls4ml was particularly interesting: it
...
cache issues and older versions of the \ac{IP} that were still being used during integration.

Another practical lesson was the importance of keeping projects well organized. In a
workflow involving Vivado, Vitis, Vitis \ac{HLS}, trained models, generated \ac{IP} cores, and
...
terminal. The \ac{IP} core returns logits, not probabilities. The confidence value is
...
\ac{LUT} and \ac{BRAM} limits of the board
\citep{han2015deep,howard2017mobilenetsefficientconvolutionalneural}.

It would also be interesting to study \ac{DSP} usage in more detail. In the final design, DSP
utilization is low, while \ac{LUT} remain the most critical resource. Future work could
focus on enforcing a more controlled mapping of multiplications to \ac{DSP} blocks, adjusting
data widths to favor that mapping, and comparing different \ac{HLS} strategies. It will not
always be possible to move logic into \ac{DSP} blocks, but this aspect deserves a deeper
exploration.

Another natural improvement would be to automate the generation flow. At present, the
process involves training the model, exporting it to hls4ml, generating the \ac{IP},
integrating it into Vivado, exporting the \ac{XSA}, and rebuilding the Vitis platform. Part
of this flow could be unified through scripts that clean older versions, regenerate the
\ac{IP}, and prepare a final reproducible folder. This would reduce cache- and path-related
...
of only individual cases. An application capable of reading images from an \ac{SD} card, or
...
consumption, since one of the reasons for using \ac{FPGA} in Edge AI is not only latency,
but also energy efficiency \citep{sze2017efficientprocessingdeepneural}.

Finally, the \ac{FPGA} accelerator could be compared against a more optimized \ac{ARM}
...
for \ac{CPU} execution. Even so, the work carried out already demonstrates the main idea:
when the computation is adapted to the hardware and properly integrated into the system,
the \ac{FPGA} can significantly reduce inference latency even on a resource-constrained
board.
