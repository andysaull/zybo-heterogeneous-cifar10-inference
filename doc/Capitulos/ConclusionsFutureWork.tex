...
only training a CIFAR-10 model, but making that model actually fit within a Zynq-7010.
...
with approximately 171 ms for the hls4ml-generated C++ core running on the ARM. This
...
platform for learning and prototyping, but the Zynq-7010 provides only a limited margin
...
utilization is low, while LUTs remain the most critical resource. Future work could
...
consumption, since one of the reasons for using FPGAs in Edge AI is not only latency,
but also efficiency \citep{sze2017efficientprocessingdeepneural}.
