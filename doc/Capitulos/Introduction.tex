...
This is where \ac{FPGA} become a key enabling technology.
Unlike traditional \ac{CPU} and \ac{GPU}, which process data either sequentially or through
fixed forms of parallelism, \ac{FPGA} allow custom hardware architectures to be designed
...
a development board that integrates the flexibility of an \ac{ARM} Cortex-A9 processor for
control-oriented tasks with the computational capabilities of a Xilinx/AMD 7-series
\ac{FPGA} for intensive processing workloads, all within the same chip. From an academic
...
is to make the most of an \ac{FPGA} with limited resources, only around 80 \ac{DSP} blocks, in
order to run Machine Learning models. This will be supported by modern
\ac{HLS} tools, which help reduce the complexity traditionally associated with
...
embedded system based on the Zynq Z-7010 \ac{SoC}, capable of accelerating the inference of a
...
inference flow on the \ac{ARM} microprocessor.
...
\begin{enumerate}
  \item \textbf{Platform bring-up:} configure the Vivado/Vitis development environment
    ...
    both the \ac{ARM} Cortex-A9 microprocessor and the \ac{FPGA}.
  \item \textbf{Communication infrastructure design:} implement high-bandwidth
    ...
    using the \ac{AMBA} \ac{AXI} protocol and \ac{DMA} controllers.
  \item \textbf{Acceleration of a baseline computation:} develop a basic linear algebra
    \ac{IP} core in \texttt{C++}, specifically a matrix multiplication accelerator, and
    synthesize it
    into hardware using \ac{HLS} in order to validate the data flow between the processor
    and the \ac{FPGA}.
  \item \textbf{\ac{AI} implementation under limited resources:} adapt a neural network model,
    specifically a \ac{CIFAR}-10 Convolutional Neural Network, to the limited resources of
    the Zynq Z-7010, generating its hardware implementation and developing the control
  ...
  \item \textbf{Performance evaluation (Benchmarking):} compare execution times, in terms
    of latency, by running the neural network inference on the \ac{CPU} alone and comparing
    it with the \ac{FPGA}-assisted execution.
\end{enumerate}
...
systems, \ac{AXI} buses, and High-Level Synthesis. Installation of the required tools
(Vivado and Vitis) and execution of a ``Hello World'' application on the board.

\textbf{Phase 2.} \ac{PS}-\ac{PL} communication: configuration of control registers
through \ac{AXI}-Lite and
setup of the memory subsystem using \ac{AXI} \ac{DMA}. Development of bare-metal C drivers to
transfer data from \ac{RAM} to the \ac{FPGA}.

\textbf{Phase 3.} Development of the baseline accelerator in \ac{HLS}: implementation of matrix
multiplication in \texttt{C}/\texttt{C++}. Use of \ac{HLS} pragmas such as PIPELINE and
UNROLL to optimize
the generated hardware. Integration of the resulting \ac{IP} block into the Vivado design
together with the \ac{DMA}.

\textbf{Phase 4.} Design and implementation of the \ac{AI} model: training and/or export of a
Convolutional Neural Network model for \ac{CIFAR}-10 image classification. Generation of
the neural network hardware accelerator and integration into the Vivado block design.

\textbf{Phase 5.} Testing and benchmarking: execution of inference on the \ac{FPGA} and comparison
against the hls4ml-generated core running on the \ac{ARM}. Collection of performance metrics
