...
static u32 output_buffer[CIFAR10_OUTPUT_WORDS] __attribute__((aligned(64)));
...
static void prepare_buffers(void)
{
    ...
    xil_printf("Input:  %u words, %u bytes\r\n",
               (unsigned int)CIFAR10_IMAGE_WORDS,
               (unsigned int)FPGA_CIFAR10_INPUT_BYTES);
    xil_printf("Output: %u words, %u bytes\r\n",
               (unsigned int)CIFAR10_OUTPUT_WORDS,
               (unsigned int)FPGA_CIFAR10_OUTPUT_BYTES);
}
...
static int run_fpga_inference_once(u32 *elapsed_time_us)
{
    ...
    status = fpga_cifar10_inference_q16(input_buffer, output_buffer);
...
int main(void)
{
    ...
    xil_printf("\r\n");
    xil_printf("============================================================\r\n");
    xil_printf(" CIFAR-10 hls4ml FPGA inference test\r\n");
    xil_printf("============================================================\r\n");
    ...
    print_results("FPGA", output_buffer);

    xil_printf("\r\nFPGA benchmark summary (%u runs)\r\n",
               (unsigned int)BENCHMARK_RUNS);
