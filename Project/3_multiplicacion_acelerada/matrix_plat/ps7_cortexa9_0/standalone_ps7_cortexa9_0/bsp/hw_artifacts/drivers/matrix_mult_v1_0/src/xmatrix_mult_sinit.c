...
XMatrix_mult_Config *XMatrix_mult_LookupConfig(UINTPTR BaseAddress) {
	...
	for (Index = (u32)0x0; XMatrix_mult_ConfigTable[Index].Name != NULL; Index++) {
		if (!BaseAddress || XMatrix_mult_ConfigTable[Index].Ctrl_BaseAddress == BaseAddress) {
...
int XMatrix_mult_Initialize(XMatrix_mult *InstancePtr, UINTPTR BaseAddress) {
	...
	ConfigPtr = XMatrix_mult_LookupConfig(BaseAddress);
	if (ConfigPtr == NULL) {
